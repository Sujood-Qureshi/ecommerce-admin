...
import { ProductForColumn } from "./column"
...
export const CellAction: React.FC<CellActionPorps> = ({ data }) => {
    ...
    const onCopy = (id: string) => {
        ...
        toast.success("Copyied!")
    }
    const onDelete = async (id: string) => {
        try {
            setLoading(true);
            await axios.delete(`/api/productsfor/${id}`);
            router.refresh()
        ...
        <AlertModal isOpen={open} onClose={()=> setOpen(false)} loading={loading} onConfirm={()=>onDelete(data.id)}/>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onClick={() => router.push(`/admin/productsfor/${data.id}`)}>
